...
3. SQL Syntax
   ...
   * 모든 SQL문(statement)는 SELECT, INSERT, UPDATE등과 같은 키워드로 시작, 하나의 statement는 세미콜론(;)으로 끝남
...
5. **Constraints**
   ...
   * **데이터 무결성**
     ...
     * 데이터베이스 내의 데이터에 대한 정확성, 일관성을 보장하기 위해 **데이터 변경 혹은 수정 시 여러 제한**을 두어 **데이터의 정확성을 보증**하는 것
   ...
   * **rowid의 특징**
     ...
     * 만약 INTEGER PRIMARY KEY 키워드를 가진 컬럼을 직접 만들면 이 컬럼은 rowid 컬럼의 별칭(alias)이 됨
       ...
       * 즉, 새 컬럼 이름으로 rowid에 엑세스 할 수 있으며, rowid이름으로도 여전히 액세스 가능
...
4. **Filtering data**
   ...
   * **SELECT DISTINCT clause**
     ...
     * 문법 규칙
       ...
       * DISTINCT 키워드 뒤 에 컬럼 또는 컬럼 목록 작성
   ...
   * **BETWEEN operator**
     ...
     * SELECT, DELETE, UPDATE 문의 WHERE절에서 사용 가능
   ...
   * 개요
     ...
     * **UPDATE**
       ...
       * 문법 규칙
         ...
         * 선택적으로 ORDER BY 및 LIMIT절을 사용하여 업데이트할 행 수를 지정 할 수도 있음
     ...
     * **DELETE**
       ...
       * 문법 규칙
         ...
         * 선택적으로 ORDER BY 및 LIMIT절을 사용하여 삭제할 행 수를 지정 할 수도 있음
