...
1. **문자열(String)**
   ...
   *  문자열 **조회/탐색 및 검증 메서드**
     ...
     | **s.isalpha()** | **알파벳 문자 여부** (단순 알파벳X 유니코드 상 letter 한국어도 포함) |
     | **s.isupper()** | **대문자** 여부                                    |
     | **s.islower()** | **소문자** 여부                                    |
     | **s.istitle()** | **타이틀 형식** 여부                                 |
   ...
   * 문자열 **변경 메서드**
     ...
     | **s.title()**                      | 문자열 내 띄어쓰기 기준으로 **각 단어의 첫글자**는 **대문자**로, **나머지**는 **소문자**로 변환 |
     | **s.upper()**                      | **모두 대문자**로 변경                                                |
     | **s.lower()**                      | **모두 소문자**로 변경                                                |
...
2. **리스트(List)**
   ...
   * 리스트 **메서드**
     ...
     | **L.extend(m)**            | **순회형 m**의 **모든 항목들**을 **리스트 끝에 추가**(+=과 같은 기능)               |
     | **L.index(x, start, end)** | 리스트 항목 중 **가장 왼쪽**에 있는 **x의 인덱스**를 반환                         |
...
1. **셋(Set)**
   ...
   * 셋 **메서드**
     ...
     | **s.update(t)**     | **셋 t**에 있는 모든 항목 중 **셋 s에 없는 항목 추가**                         |
     | **s.clear()**       | **모든 항목 제거**                                                  |
